...
class SideDrawer(QWidget):
    ...
    def __init__(self, *args, stretch=1 / 3, direction=0, widget=None, **kwargs):
        super(SideDrawer, self).__init__(*args, **kwargs)
    ...
    def resizeEvent(self, event):
        ...
        super(SideDrawer, self).resizeEvent(event)
    ...
    def mousePressEvent(self, event):
        ...
        if pos.x() >= 0 and pos.y() >= 0 and self.childAt(pos) is None and self.widget:
            if not self.widget.geometry().contains(pos):
                self.animationOut()
                return
        super(SideDrawer, self).mousePressEvent(event)

    def show(self):
        super(SideDrawer, self).show()
